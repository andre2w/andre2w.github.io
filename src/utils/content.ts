...
function isVisiblePost(post: Post): boolean {
  ...
  const isFuturePost = new Date(post.data.published).getTime() > Date.now();
...
function sortPosts(posts: Post[]): Post[] {
  return posts.sort((a, b) => {
    const aDate = new Date(a.data.updated ?? a.data.published).getTime();

    const bDate = new Date(b.data.updated ?? b.data.published).getTime();
...
export async function getAllPages(): Promise<Page[]> {
  ...
  const pages = await getCollection("pages", (page) => isVisiblePage(page) && isPublicPage(page));
 ...
 * -> ["travel", "japan"]
 */
export function getPostPathSegments(filePath?: string): string[] {
  if (!filePath) {
    return [];
  }
 ...
 * -> "travel/japan/tokyo"
 */
export function getPostSlugPath(id: string, filePath?: string): string {
...
export function getPagePathSegments(filePath?: string): string[] {
  if (!filePath) {
    return [];
  }
...
export function getPageSlugPath(id: string, filePath?: string): string {
...
export function getPageSlug(id: string, filePath?: string): string {
 ...
 */
export function getPostUrl(id: string, filePath?: string): string {
 ...
 * Get adjacent posts.
 */
export function getAdjacentPosts<T extends Post>(posts: T[], currentPost: T) {
...
export function getPostsGroupedByYear(entries: Post[]): [string, Post[]][] {
  ...
  for (const year in grouped) {
    grouped[year].sort((a, b) => b.data.published.valueOf() - a.data.published.valueOf());
  }
